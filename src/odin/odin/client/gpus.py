...
Row = namedtuple('Row', 'host gpu type free util memuse processes pids ')
...
def _gpu2row(gpu, host):
    ...
    row = Row(host=host, gpu=gpu_id, type=gpu_type, free=free, util=util, memuse='{}M'.format(used_memory) if used_memory else '', processes=processes, pids=pids)
    return row
